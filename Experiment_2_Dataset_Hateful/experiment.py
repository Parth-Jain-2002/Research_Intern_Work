        ...
        for test_size in test_sizes:
            for model_name in tqdm(model_names, desc='Models', leave=True):
                ...
                for i in tqdm(range(n_runs), leave=False, desc='Runs'):
                    ...
                    combiner.fit(model_probs_tr, y_h_tr, y_true_tr, num_humans = NUM_HUMANS)
                    ...
                    for policy_name, policy, use_true_labels in [('model_prob_based_policy',model_prob_based_policy,False)]:
                        humans = policy(combiner, model_probs_te, NUM_HUMANS, model_probs_te.shape[1])
                        y_comb_te = combiner.combine(model_probs_te, y_h_te, humans)
                        acc_comb = get_acc(y_comb_te, y_true_te)
                        _acc_data.append(acc_comb)
            ...
            header_acc = ['human', 'model'] + [policy_name for policy_name, _, _ in POLICIES] + ['model_prob_based_policy']
